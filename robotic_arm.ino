...
void handleWebSocketMessage(void *arg, uint8_t *data, size_t len) {
  ...
  if (info->final && info->index == 0 && info->len == len && info->opcode == WS_TEXT) {
    ...
    if (message.indexOf("1s") >= 0) {
      ...
      dutyCycle1 = map(sliderValue1.toInt(), 0, 100, 180, 0);
...
//  ledcAttachPin(leftPin, ledChannel1);
//  ledcAttachPin(rightPin, ledChannel2);
//  ledcAttachPin(basePin, ledChannel3);
...
void loop() {
  left.write(dutyCycle1);
  right.write(dutyCycle2);
  base.write(dutyCycle3);
  grip.write(dutyCycle4);
